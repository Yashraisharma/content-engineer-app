...
with st.sidebar:
    ...
    keywords_input = st.text_input("Core Keywords", placeholder="e.g. Refill, Insulin, Safety", key="kw_sidebar_v2")
    prod_description = st.text_area("Product Details", height=150, key="prod_sidebar_v2")
    intention = st.text_area("Primary Goal", height=150, key="goal_sidebar_v2")

    st.divider()
    
    st.header("🔍 Advanced Targeting (Optional)")
    seg_type = st.text_input("1. Segment Type", placeholder="e.g. Lapsed Users", key="type_sidebar_v2")
    seg_reason = st.text_input("2. Reason for Segment", placeholder="e.g. 30-Day Inactive", key="reason_sidebar_v2")
    sub_seg = st.text_input("3. Sub Segment", placeholder="e.g. Chronic/Insulin", key="sub_sidebar_v2")
