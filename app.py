...
with st.sidebar:
    ...
    keywords_input = st.text_input("Target Keywords", placeholder="e.g. BOGO, Sale", key="v13_kw")
    prod_description = st.text_area("Product Details", height=120, key="v13_prod")
    intention = st.text_area("Primary Goal", height=120, key="v13_goal")
    
    st.divider()
    st.header("🔍 Advanced Segmentation")
    seg_type = st.text_input("Segment Type", key="v13_type")
    seg_reason = st.text_input("Reason for Segment", key="v13_reason")
    sub_seg = st.text_input("Sub Segment", key="v13_sub")
    spec_prod = st.text_input("Specific Product Base", key="v13_base")

    st.divider()
    st.header("📋 Project Summary")
    st.write("This tool identifies proven success patterns from your raw CSV data and evolves them into 10 high-performing variations.")
    ...
    with st.expander("1. Firm Ranking Engine", expanded=True):
        ...
        st.caption("Prioritizes high-CTR creative validated by Impression scale.")

    with st.expander("2. 7+3 Engineering Strategy", expanded=True):
        st.write("**Evolutionary (7 Rows):** Performance-led skeletons.")
        st.write("**Revolutionary (3 Rows):** New psychological hooks.")
...
perf_files = st.file_uploader("Upload Historical Performance CSVs", type="csv", accept_multiple_files=True, key="v13_perf_up")
...
if perf_files:
    try:
        ...
        view_idx = next((i for i, c in enumerate(cols) if any(x in c for x in ['viewed', 'sent', 'impression', 'delivered'])), None)
        ...
        if view_idx is not None and click_idx is not None:
            ...
            df_p['True_CTR_Perc'] = (df_p['C_N'] / df_p['V_N'].replace(0, np.nan)) * 100
            df_p['True_CTR_Perc'] = df_p['True_CTR_Perc'].fillna(0.0)
            
            # Ranking Logic
            df_p['Power_Score'] = (df_p['True_CTR_Perc'] * 0.7) + ((df_p['V_N'] / (df_p['V_N'].max() or 1)) * 30)
            
            full_ranked = df_p.sort_values(by='Power_Score', ascending=False)
            full_ranked['CTR% Display'] = full_ranked['True_CTR_Perc'].apply(lambda x: f"{x:.2f}%")
            
            t1, t2 = st.tabs(["📑 All Uploaded Rows (Ranked)", "🏆 Top 10 Winners"])
...
format_file = st.file_uploader("Upload Format Template CSV", type="csv", key="v13_format_up")
...
if format_file:
    ...
    if st.button("🚀 Run Stream 2 Strategic Engineering (File 2 Style)", key="v13_btn_f"):
        if not ACTIVE_KEY:
